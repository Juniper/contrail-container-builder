...
function get_mask_net_for_nic() {
  local nic=$1
  get_routes | awk -v nic=$nic '{if($1==nic){split($2,a,"/");if(a[2]!=0){printf("%s %s\n",a[2],a[1])}}}' | sort -n | head -1
}

function get_ip_for_nic() {
  local nic=$1
  local mask_net=`get_mask_net_for_nic $nic`
  local mask=`echo $mask_net | cut -d ' ' -f 1`
  local net=`echo $mask_net | cut -d ' ' -f 2`
  awk -v cidr="$net/$mask" -v flag="0" '{if($2 == cidr){flag=1}; if(flag==1 && $1=="/32" && $2=="host"){print f;flag=0} {f=$2}}' /proc/net/fib_trie | head -1
}

function get_subnet_for_nic() {
  local nic=$1
  local mask_net=`get_mask_net_for_nic $nic`
  echo $mask_net | cut -d ' ' -f 2
}
