...
function provision() {
  ...
  for (( i=0 ; i < retries ; ++i )) ; do
    ...
    for server in $servers ; do
      if python /opt/contrail/utils/$script $rest_params --api_server_ip $server --api_server_port $CONFIG_API_PORT $AUTH_PARAMS --api_server_use_ssl ${CONFIG_API_SSL_ENABLE-False} ; then
