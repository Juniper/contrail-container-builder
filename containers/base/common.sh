...
if [[ $CLOUD_ORCHESTRATOR == 'openstack' ]] ; then
  LINKLOCAL_ENTRY_DEFAULT="${LINKLOCAL_SERVICE_NAME:-'metadata'}:${LINKLOCAL_SERVICE_IP:-'169.254.169.254'}:${LINKLOCAL_SERVICE_PORT:-80}:${IPFABRIC_SERVICE_HOST:-${KEYSTONE_AUTH_HOST}}:${IPFABRIC_SERVICE_PORT:-8775}"
  LINKLOCAL_MAP=${LINKLOCAL_MAP:-${LINKLOCAL_ENTRY_DEFAULT}}
else
  LINKLOCAL_MAP=${LINKLOCAL_MAP:-}
fi
