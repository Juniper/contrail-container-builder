...
if [[ -e /etc/sysconfig/network-scripts/ifcfg-${phys_int} && ! -e /etc/sysconfig/network-scripts/ifcfg-vhost0 ]]; then
    ...
    sed -i '/IPADDR=/d' /etc/sysconfig/network-scripts/ifcfg-${phys_int}
    sed -i '/NETMASK=/d' /etc/sysconfig/network-scripts/ifcfg-${phys_int}
    sed -i '/GATEWAY=/d' /etc/sysconfig/network-scripts/ifcfg-${phys_int}
    sed -i '/DNS/d' /etc/sysconfig/network-scripts/ifcfg-${phys_int}
    sed -i '/BOOTPROTO/d' /etc/sysconfig/network-scripts/ifcfg-${phys_int}
    sed -i '/NETWORK/d' /etc/sysconfig/network-scripts/ifcfg-${phys_int}
    sed -i '/BROADCAST/d' /etc/sysconfig/network-scripts/ifcfg-${phys_int}
    sed -i '/MACADDR/d' /etc/sysconfig/network-scripts/ifcfg-${phys_int}
    ifdown ${phys_int}
    ifup ${phys_int}
    ifup vhost0
