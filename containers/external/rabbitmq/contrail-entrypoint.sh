...
if [[ -n "$RABBITMQ_MIRRORED_QUEUE_MODE" ]] ; then
  ...
  salt=$(cat /dev/urandom | tr -dc 'A-F0-9' | fold -w 8 | head -n 1)
  salt_pwd="${salt}${RABBITMQ_PASSWORD}"
  sha256="$(echo $salt_pwd | sha256sum | cut -d ' ' -f1)"
  salt_sha256="${salt}${sha256}"
  b64="$(echo $salt_sha256 | base64 -w 0)"
