...
if [[ ${KAFKA_SSL_ENABLE} == "False" ]] ; then
    ...
    grep -q '^ssl.keystore.location' ${CONFIG} && sed -i 's/^ssl.keystore.location.*$/ssl.keystore.location=\/var\/private\/ssl\/kafka.server.keystore.jks/' ${CONFIG}  || echo "ssl.keystore.location=\/var\/private\/ssl\/kafka.server.keystore.jks" >> ${CONFIG}
    grep -q '^ssl.truststore' ${CONFIG} && sed -i 's/^ssl.truststore.*$/ssl.truststore=\/var\/private\/ssl\/kafka.server.truststore.jks/' ${CONFIG}  || echo "ssl.truststore=\/var\/private\/ssl\/kafka.server.truststore.jks" >> ${CONFIG}
    grep -q '^ssl.keystore.password' ${CONFIG} && sed -i 's/^ssl.keystore.password.*$/ssl.keystore.password=c0ntrail123/' ${CONFIG}  || echo "ssl.keystore.password=c0ntrail123" >> ${CONFIG}
    ...
    grep -q '^ssl.truststore.password' ${CONFIG} && sed -i 's/^ssl.truststore.password.*$/ssl.truststore.password=c0ntrail123/' ${CONFIG}  || echo "ssl.truststore.password=c0ntrail123" >> ${CONFIG}
