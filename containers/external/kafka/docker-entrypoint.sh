...
if [[ ${KAFKA_SSL_ENABLE} == "False" ]] ; then
    ...
    (grep -q '^advertised.listeners' ${CONFIG} && sed -i "s|^advertised.listeners.*$|advertised.listeners=SSL:\/\/$KAFKA_LISTEN_ADDRESS:$KAFKA_SSL_PORT|" ${CONFIG}) || echo "advertised.listeners=SSL:\/\/$KAFKA_LISTEN_ADDRESS:$KAFKA_SSL_PORT" >> ${CONFIG}
