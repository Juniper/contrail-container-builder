...
vnc_api = VncApi(username=os.environ['KEYSTONE_AUTH_ADMIN_USER'],
             ...
             api_server_host=(os.environ['CONTROL_NODES']).split(','))
...
def main():
    ...
    if sys.argv[1] == 'read':
        ...
        logger.info("DNSMASQ_LEASE_OBTAINED=0 %s %s" % (sys.argv[2], sys.argv[3]))
        logger.info("Router created : %s" % fq_name)
