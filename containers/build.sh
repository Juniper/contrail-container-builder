...
function process_container() {
  ...
  log "Building $container_name" | append_log_file $logfile
  ...
  if [ ${exit_code} -eq 0 ]; then
    ...
    log "Building $container_name finished successfully" | append_log_file $logfile
  else
    log "Building $container_name failed" | append_log_file $logfile
    was_errors=1
  fi
  return $exit_code
}
