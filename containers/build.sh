...
[ "${CONTRAIL_BUILD_FROM_SOURCE}" -eq 'true' ] && log "Contrail source root: $CONTRAIL_SOURCE"
...
function process_container() {
  ...
  if [[ ${exit_code} -eq 0 && "${CONTRAIL_BUILD_FROM_SOURCE}" -eq 'true' && -e ${relative_build_src_path} ]]; then
    # Setup from source
