...
function process_container() {
  ...
  if [[ ${exit_code} -eq 0 && ! -z "$CONTRAIL_BUILD_FROM_SOURCE" && -e ${relative_build_src_path} ]]; then
    ...
    ########
    if [[ "${DISTROLESS_ENABLED}" == 'true' and ${docker_ver} > '17.05' ]] ; then
      #get parent image for compare files from parent and new image
      local parent_image=$(grep -E "^FROM" ${docker_file}  | awk '{print $2}')
      local parent_container="container-${parent_image}"
      log "The parent image is ${parent_image}" | append_log_file $logfile
      ...
      if [ ${exit_code} -eq 0 ]; then
        ...
        docker diff ${parent_container} $intermediate_base > ${files_different} | append_log_file $logfile
      fi
