...
function process_container() {
 ...
 # For setup from sources: base dependencies (rpms) to avoid 
  ...
  local docker_command1=''
  docker_command1+="docker build --network host -t ${target_name} -t ${target_name_os} ${build_arg_opts} -f ${docker_file} ${opts} ${dir}"
  log "Docker command is: ${docker_command1}" | append_log_file ${logfile} true
  bash -c "${docker_command1}" 2>&1 | append_log_file $logfile true
