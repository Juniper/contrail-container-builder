...
function process_container() {
  ...
  if [[ -f ./$dir/.externals ]]; then
    ...
    for item in `cat ./$dir/.externals` ; do
      ...
      rsync -r --exclude $dst --exclude-from='../.gitignore' ./$dir/$src ./$dir/$dst 2>&1 | append_log_file
    done
  fi
  ...
  if [ $exit_code -eq 0 -a ${CONTRAIL_REGISTRY_PUSH} -eq 1 ]; then
    docker push ${CONTRAIL_REGISTRY}'/'${container_name}:${tag} 2>&1 | append_log_file -a $logfile
    exit_code=${PIPESTATUS[0]}
    # temporary workaround; to be removed when all other components switch to new tags
    docker push ${CONTRAIL_REGISTRY}'/'${container_name}:${OPENSTACK_VERSION}-${tag} 2>&1 | append_log_file -a $logfile
  fi
