...
function process_container() {
  ...
  #TODO: host path is needed. need to fix in  tf_dev_env
  local current_abs_path='/var/lib/docker/overlay2/3ea20d9657642e284e4ef433fc233c63cb5c9e264ec4119aff3caafe9e768570/diff/root/src/review.opencontrail.org/Juniper/contrail-container-builder/containers'
  local build_src_dir="$current_abs_path/$dir/build_src"
  
  if [[ ${exit_code} -eq 0 && ! -z "$CONTRAIL_BUILD_FROM_SOURCE" && "$(ls -A ./$dir/build_src)" ]]; then
    # Setup from source
    ...
    docker run --name $intermediate_base --network host \
       ...
       -v ${build_src_dir}:/build_src \
       --entrypoint /build_src/setup \
