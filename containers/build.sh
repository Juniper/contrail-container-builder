...
function process_container() {
  ...
  if [ ${PIPESTATUS[0]} -eq 0 ]; then
    ...
    if [ ${PIPESTATUS[0]} -eq 0 ]; then
      docker tag ${CONTRAIL_REGISTRY}'/'${container_name}:${CONTRAIL_CONTAINER_TAG} ${CONTRAIL_REGISTRY}'/'${container_name}:${CONTRAIL_CONTAINER_TAG_OLD}
      docker push ${CONTRAIL_REGISTRY}'/'${container_name}:${CONTRAIL_CONTAINER_TAG_OLD}
      rm $logfile
    fi
  fi
  if [ -f $logfile ]; then
