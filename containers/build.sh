...
function process_container() {
  ...
  if [[ -n "$CONTRAIL_SOURCE_ROOT_HOST" && -e ./$dir/setup.sh ]] ; then
    ...
    # setup from RPMs
    contrail-config contrail-config-openstack
    local rpm_items=''
    if [[ -e ./$dir/.rpm ]]; then
      rpm_items=$(cat ./$dir/.rpm | sed '/^$/d' | tr '\n' ',')
      build_arg_opts+=" --build-arg CONTRAIL_RPMS=\"${rpm_items}\""
    fi
    log "Setup Contrail from RPMs: $CONTRAIL_RPMS" | append_log_file $logfile true
  fi
