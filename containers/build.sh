...
function process_container() {
  ...
  if [[ "$docker_ver" < '17.06' ]] ; then
    # hack change for all inherited from general-base (change FROM string and remove ARGS before)
    cat ${docker_file} | tr '\n' '\f' | \
      sed -e 's|ARG.*\fARG.*\fFROM ${CONTRAIL_REGISTRY}/\([^:]*\):${CONTRAIL_CONTAINER_TAG}|FROM '${CONTRAIL_REGISTRY}'/\1:'${tag}'|' | \
      tr '\f' '\n' > ${docker_file}.nofromargs
    # simple change for general-base
    sed -i \
