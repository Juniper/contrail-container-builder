...
if [[ -z "$my_image" ]]; then
  echo "Self docker image couldn't be evaluated. Please provide IMAGE variable from outside to create /usr/bin/contrail-status file."
else
  cat > /host/usr/bin/contrail-status << EOM
#!/bin/bash -e
docker run --rm --name contrail-status -v /usr/bin:/host/usr/bin -v /var/run/docker.sock:/var/run/docker.sock --pid host --net host --privileged --env IMAGE=$my_image $my_image
EOM
