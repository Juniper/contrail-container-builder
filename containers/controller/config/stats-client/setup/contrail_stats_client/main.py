...
class Scheduler(object):
    ...
    def _get_updated_send_freq():
        ...
        for tag in init_vnc_api_client().tags_list()["tags"]:
            tag = tag["fq_name"][0]
...
class Postman(object):
    ...
    def __init__(self):
        ...
        self._parse_config()
        self.logger = self._init_logger(log_level=self._log_level,
    ...
    def send_stats(self, stats=Stats(client=init_vnc_api_client()), stats_serv=getenv('STATS_SERVER')):
        ...
        resp = {}
        try:
            resp_code = urlopen(url=Request(url=stats_serv, data=json.dumps(stats.__dict__), headers={'Content-Type': 'application/json'})).code
...
def main():
    ...
    postman = Postman()
    while True:
        postman.logger.info("TF usage report client started. Frequency of statistics sending is %s" % (str(scheduler.send_freq)))
        if scheduler.is_job():
            postman.send_stats()
            scheduler.job_is_done()
        postman.logger.info("Statistics sending is scheduled at %s" % str(scheduler.sched_job_ts))
