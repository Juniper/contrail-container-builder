...
cat > $tmp_file << EOM
#!/bin/bash -e
docker run --rm --name \${CONTRAIL_STATUS_CONTAINER_NAME:-contrail-status} $vol_opts $env_opts --pid host --net host --privileged ${CONTRAIL_STATUS_IMAGE} /root/contrail-status.py ${CONTRAIL_STATUS_OPTS} \$@
EOM
