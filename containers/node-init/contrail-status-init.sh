...
name=${CONTRAIL_STATUS_CONTAINER_NAME:-contrail-status}
tmp_suffix="--name $name --rm --pid host --net host --privileged ${CONTRAIL_STATUS_IMAGE} /root/contrail-status.py ${cmd_args} \$@"
tmp_file=/host/usr/bin/contrail-status.tmp
cat > $tmp_file << EOM
#!/bin/bash
u=\$(which docker >/dev/null 2>&1)
if pidof dockerd >/dev/null 2>&1 || pidof dockerd-current >/dev/null 2>&1 ; then
    \$u run $vol_opts $tmp_suffix
    exit \$?
fi
u=\$(which podman >/dev/null 2>&1)
if ((\$? == 0)); then
    r="\$u run $vol_opts "
