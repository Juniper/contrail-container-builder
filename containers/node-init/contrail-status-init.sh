...
if ((\$? == 0)); then
    ...
    if ((0 == \$?)) && ((\$k > 0)) && [[ -S /var/run/crio/crio.sock ]]; then
        r+=' --volume=/var/run/crio/crio.sock:/var/run/crio/crio.sock'
    else
        r+=' --volume=/sys/fs/selinux:/sys/fs/selinux --volume=/var/lib/containers:/var/lib/containers --volume=/var/run/libpod:/var/run/libpod --volume=/run/runc:/run/runc --volume=/sys/fs/cgroup/:/sys/fs/cgroup/ --cap-add=ALL --security-opt seccomp=unconfined --pid host '
    fi
    \$r $tmp_suffix
    exit \$?
fi
