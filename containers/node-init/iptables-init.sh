...
function get_iptables_ports_to_configure() {
  ...
  if [[ -z "$CONTRAIL_SVC_NAME" ]]; then
    ...
    case "$CONTRAIL_SVC_NAME" in
        "vrouter")
            ports="$VROUTER_AGENT_INTROSPECT_PORT,$VROUTER_AGENT_METADATA_PROXY_PORT,$VROUTER_AGENT_IPC_PORT,$VROUTER_AGENT_NODEMGR_PORT";;
        "redis")
        ...
        "contrail_webui")
            ports="$WEBUI_HTTPS_LISTEN_PORT,$WEBUI_HTTP_LISTEN_PORT,$WEBUI_CONSOLE_PORT";;
        "rabbitMQ")
            ports="$RABBITMQ_ARB_PORT_1,$RABBITMQ_ARB_PORT_2,$RABBITMQ_ARB_PORT_3";;
        ...
        "kubemanager")
            ports="$KUBEMANAGER_INTROSPECT_PORT";;
        *)
            ports=''
