...
RABBITMQ_NODE_TLS_PORT=${RABBITMQ_NODE_TLS_PORT:-`expr $RABBITMQ_NODE_PORT + 20000`}
...
case "$NODE_TYPE" in
    ...
    "config-database")
        ...
        ports+="$RABBITMQ_EPMD_PORT $RABBITMQ_NODE_TLS_PORT $RABBITMQ_NODE_PORT";;
    "config")
...
if [[ -z $(iptables --list $IPTABLES_CHAIN -t $IPTABLES_TABLE -n) ]]; then
...
for port in $ports; do
  ...
  if  [[ -z $(iptables --list $IPTABLES_CHAIN -t $IPTABLES_TABLE -n | grep -o -E 'dpt:[0-9]+' | cut -f 2 -d ':' | grep -qw $port) ]] ; then
    ...
    if [[ -z $(iptables -t $IPTABLES_TABLE -I $IPTABLES_CHAIN 1 -w 5 -W 100000 -p tcp --dport $port -j ACCEPT) ]; then
