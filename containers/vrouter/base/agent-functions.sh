...
function remove_vhost0() {
    ...
    if ! is_dpdk ; then
        ...
        remove_vhost0_dpdk || { echo "ERROR: failed to remove vhost0" && return 1; }
        restore_iface_driver $pci $DPDK_UIO_DRIVER || { echo "ERROR: failed to rebind to net driver" && return 1; }
    fi
