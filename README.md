...
* It is mandatory to pass variable KUBE_MANIFEST to "docker run" command. In order to obtain acceptable KUBE_MANIFEST values - run the command without parameters and a list of values will be generated to output. For example:<br/>
``` docker run --rm $CONTRAIL_REPO_IMAGE```

* To generate a deployment yaml and outputs it in contrail.yaml - run the command:<br/>
